...
import { Settings2, ChevronDown, ChevronUp, Save, CheckCircle } from 'lucide-react'
...
export function FormConfigEditor({ facilityId, currentConfig }: Props) {
  ...
  const handleSave = async () => {
    ...
    const res = await fetch('/api/facilities', {
      ...
      body: JSON.stringify({ id: facilityId, form_config: config }),
    })
    setSaving(false)
    if (res.ok) {
      setSaved(true)
    ...
    <div className="border border-gray-200 rounded-xl overflow-hidden">
      ...
      >
        <div className="flex items-center gap-2">
          ...
          <span className="text-sm font-medium text-gray-600">フォーム項目の設定</span>
        </div>
        {open
